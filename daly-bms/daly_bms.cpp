...
// Include per avere la definizione completa di DalyBmsDispatcher
...
void DalyBmsComponent::setup() { 
  ...
  // Registra questa istanza con il dispatcher
  if (this->dispatcher_ != nullptr) {
    this->dispatcher_->register_bms(this->response_addr_, this);
    ESP_LOGCONFIG(TAG, "Registered with dispatcher: addr=0x%02X, response_addr=0x%02X", 
                  this->addr_, this->response_addr_);
  } else {
    ESP_LOGE(TAG, "ERRORE: Dispatcher not set! Multi-BMS setup will NOT work!");
  }
}
...
void DalyBmsComponent::dump_config() {
  ...
  ESP_LOGCONFIG(TAG, "  Response Address: 0x%02X", this->response_addr_);
  // Non possiamo chiamare check_uart_settings perché non ereditiamo da UARTDevice
  // L'UART è gestito dal dispatcher
}
...
void DalyBmsComponent::loop() {
  ...
  // Debug log ogni 30 secondi per confermare che la patch è attiva
  if (now - this->last_debug_log_ > 30000) {
    ESP_LOGI(TAG, "PATCH ATTIVA - Addr:0x%02X RespAddr:0x%02X", this->addr_, this->response_addr_);
    this->last_debug_log_ = now;
  }
  
  // NON leggiamo più dall'UART qui - lo fa il dispatcher!
  // Rimuoviamo tutto il codice di lettura UART
...
void DalyBmsComponent::request_data_(uint8_t data_id) {
  ...
  // Usa il dispatcher per inviare invece di scrivere direttamente
  if (this->dispatcher_ != nullptr) {
    this->dispatcher_->write_frame(request_message, sizeof(request_message));
  } else {
    ESP_LOGE(TAG, "Cannot send request: dispatcher is null!");
  }
}
...
void DalyBmsComponent::decode_data_(std::vector<uint8_t> data) {
  ...
  while ((it = std::find(it, data.end(), 0xA5)) != data.end()) {
    // CRITICAL FIX: Check against configured response_addr_ instead of hardcoded 0x01
    if (data.end() - it >= DALY_FRAME_SIZE && it[1] == this->response_addr_) {
      ESP_LOGV(TAG, "ACCEPTING frame with address 0x%02X (my response_addr_=0x%02X)", it[1], this->response_addr_);
      uint8_t checksum;
      int sum = 0;
      for (int i = 0; i < 12; i++) {
        sum += it[i];
      ...
      // Log ignored frames with wrong address (only at VERBOSE level to avoid spam)
      if (data.end() - it >= DALY_FRAME_SIZE && it[0] == 0xA5) {
        ESP_LOGV(TAG, "Ignoring frame with address 0x%02X (expecting 0x%02X)", it[1], this->response_addr_);
      }
      std::advance(it, 1);
    }
  }
...
void DalyBmsComponent::process_frame(const std::vector<uint8_t> &frame) {
  ESP_LOGV(TAG, "Processing frame for BMS addr=0x%02X, response_addr=0x%02X", 
           this->addr_, this->response_addr_);
  
  // Aggiorna il timestamp
  this->last_transmission_ = App.get_loop_component_start_time();
  
  // Processa il frame
  this->decode_data_(frame);
}
